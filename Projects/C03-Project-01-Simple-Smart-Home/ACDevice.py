    ...
    def _on_message(self, client, userdata, msg):
        ...
        if (Edge_command == "ACK_Registeration") and (self.GotAck == "FALSE"):
            ...
            if (data["SetIntensity"] != "NOT_EXITS"):
                # Prepare OUTGOING ERROR MESSAGE DATA
                print(f"---------->>>>>>>>>> DEVICE TO EDGE ERROR Device:{self._device_id} , Type:{self._device_type} , Room:{self._room_type} CMD:{Edge_Command} REPLY\n")
                print(f"STATUS ERROR WRONG COMMAND TO AC Device:{device_reg_id} , Type:{device_type} , Room:{device_room} ,SwitchState:{self._switch_status} Device Intensity:{self._get_temperature()} CMD:{Edge_command} CALL_REASON:{Call_Reason} REPLY\n")
            ...
            # print(f"---------->>>>>>>>>> DEVICE TO EDGE Device:{self._device_id} , Type:{self._device_type} , Room:{self._room_type} CMD:{Edge_Command} CALL_REASON:{Call_Reason} REPLY\n")
            print(
                f"STATUS Device:{device_reg_id} , Type:{device_type} , Room:{device_room} ,SwitchState:{self._switch_status} Device Temprature:{self._get_temperature()} CMD:{Edge_command} CALL_REASON:{Call_Reason} \n")
            time.sleep(1)
            self.client.publish("devices", json.dumps(message))
