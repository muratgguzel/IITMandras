...
class AC_Device():
    ...
    def _on_message(self, client, userdata, msg):
        ...
        if (Edge_command=="ACK_Registeration") and (self.GotAck=="FALSE"):
            ...
            self.GotAck="TRUE"
        elif (Edge_command == "GET_STATUS") and (device_id==self._device_id):
            ...
            Edge_Command = data["Edge_Command"]
            print(f"---------->>>>>>>>>> DEVICE TO EDGE Device:{self._device_id} , Type:{self._device_type} , Room:{self._room_type} , Room_Temp:{self._temperature} CMD:{Edge_Command} REPLY\n")
            print(f"STATUS Device:{device_reg_id} , Type:{device_type} , Room:{device_room},Room_Temp:{self._temperature} ,SwitchState:{self._switch_status}  CMD:{Edge_Command} REPLY\n")
            time.sleep(1)
